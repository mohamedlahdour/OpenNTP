...
!BL10
subroutine flux_guess(dim,ng,Nmat,nregion,ngauss,order,totNFM,NusigF,dcell,wt,p,flux_ni,flux_li)
       implicit none
       ...
       integer(kind=4) :: i,k1,n,l,m,k0,k2
       real(kind=8), dimension(ng*nregion) :: a10,a11
       real(kind=8), dimension(dim,ngauss*ng) :: flux1
       ...
       do k1 = 1,ng
          do i=k0,totNFM*k1
             do n = 1,ngauss
                l = 0
                do m = 1,order
                   flux1(i,n+k2-1) = flux1(i,n+k2-1) + 0.5*(2*float(l)+1)*flux2(i,m)*p(m,n)
                   l = l+1
                enddo 
             enddo  
          enddo
       ...
       enddo
                 ...
                 do k1 = 1,ng
                    do i  = k1*totNFM,k0,-1
                       do n = 1,(ngauss/2)
                          flux2(i,:) = flux2(i,:) + wt(n)*flux1(i,n+k2-1)*p(:,n)
                       enddo
                    enddo 
...
subroutine current_f(BC,A,B,Q_ni,curr,ng,ngauss,dim,totNFM)
       ...
       real(kind=8), dimension(dim,ngauss*ng), intent(in) :: A,B,Q_ni
...
! ==============================================================================
             ...
             do k1 = 1,ng
                do n =1,ngauss/2
                   do i = k1*totNFM,k0,-1
                      phi(i) = phi(i) + wt(n)*flux_ni(i,n+k2-1)
                   enddo  
                enddo
                do n = (ngauss/2)+1,ngauss
                   do i = k0,k1*totNFM
                ...
                k2 = k2 + ngauss
             enddo
             err_phi =  maxval(abs(flux_ni-flux00)/flux00)
             it = it + 1     
